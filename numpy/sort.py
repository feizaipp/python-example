...
# NumPy 排序、条件刷选函数
# NumPy 提供了多种排序的方法。 
# 这些排序函数实现不同的排序算法，每个排序算法的特征在于执行速度，
# 最坏情况性能，所需的工作空间和算法的稳定性。 下表显示了三种排序算法的比较。
# 种类	                         速度	     最坏情况	工作空间	稳定性
...
# 在总成绩相同时，数学成绩高的优先录取，在总成绩和数学成绩都相同时，
...
print ('调用 nonzero() 函数：')
print (np.nonzero (a))
# 输出为： (array([0, 0, 1, 1, 2, 2]), array([0, 1, 1, 2, 0, 2]))
# 意思是： a[0,0], a[0,1], a[1,1], a[1,2], a[2,0] a[2,2] 不为零
